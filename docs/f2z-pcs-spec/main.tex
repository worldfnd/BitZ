...
\definecolor{spectableteal}{HTML}{2D6F67}
\definecolor{spectablerow}{HTML}{F7F9FC}
\definecolor{spectableline}{HTML}{A9BACB}
...
\fancyhead[R]{\footnotesize\sffamily\color{specmuted}integer lifting / recursive Ligerito}
\fancyfoot[C]{\thepage}
...
\]
where $\bm f$ is the committed binary witness, $\bm h$ is the virtual witness
used by the PIOP, and $\bm M$ is the effective virtualization map.
Virtualization is optional: when the developer omits it, $\bm h=\bm f$ and
$\bm M=I$.
Here $\pi_2:\mathbb Z\to\Ftwo$ denotes coordinatewise reduction modulo $2$.
The PIOP supplies this linear claim to F2Z.
...
\begin{itemize}[leftmargin=1.4em]
 ...
 \item \textbf{Witness virtualization.}
       When configured, the public map $\bm M$ relates the committed witness
       $\bm f$ to the PIOP-facing witness $\bm h$. This permits a
       constraint-specific witness layout while retaining one commitment;
       the direct configuration uses the identity map.

 \item \textbf{Input-claim shaping.}
       A degree-two LinCheck over $\Fq$ reduces the arbitrary public
       $\bm v$-weighted claim to one evaluation-shaped claim.  The resulting
       row and column equality weights are verifier-derived and are the only
       factors consumed by the integer-fold core.

 \item \textbf{Bounded integer lifting.}
       Coefficients in $\Fq$ are lifted to their canonical integers.  The
       configured row dimension guarantees that every resulting integer fold
       lies within the injective range of the binary-field exponent encoding.
 ...
 \item \textbf{GKR and LinCheck reductions.}
       ...
       evaluation of the virtual witness.
 ...
 \item \textbf{Adjoint pullback.}
       ...
       protocol message. This step is the identity when virtualization is
       omitted.

 \item \textbf{Structured ring switching.}
       The pulled-back coefficient is decomposed across the $128$-bit packing
       boundary.  The ring switch then converts the resulting inner product on
       $\bm f$ directly into one packed linear claim for the recursive Ligerito
       opening; no post-adjoint LinCheck is required.
\end{itemize}

Together, these techniques allow F2Z to prove inner-product claims over $\Fq$, and
