...
\end{equation}
Merkle compilation commits to the oracle in
Equation~\eqref{eq:encoded-oracle-binding} with the public root
...
\end{equation}
Every reduction below uses the notation fixed by
Equation~\eqref{eq:reduction-edge-notation}.  Each subsection defines one source relation and lists its direct outgoing
...
\end{align}
Equations~\eqref{eq:rel-pimsat-unencoded} and~\eqref{eq:rel-pimsat-app}
distinguish the semantic witness relation from its encoded-oracle form.
...
\begin{equation}
 ...
 \RPIMSAT^{\rm enc}.
 \label{eq:edge-encode-witness}
\end{equation}
Along the edge in Equation~\eqref{eq:edge-encode-witness}, the IOP supplies the
encoded oracle $O$; Merkle compilation exposes
...
\begin{equation}
 ...
 \RPESAT^{\rm app}.
 \label{eq:edge-application-pesat}
\end{equation}
\begin{itemize}
 \item \textbf{Proof:} On the edge in
 Equation~\eqref{eq:edge-application-pesat}, $\pi_{{\rm app},1}$ carries the
 PIOP messages that turn
...
\end{align}
Equation~\eqref{eq:rel-pesat-app} is the source relation for the application
linearization below.
...
\begin{equation}
 ...
 \RLMFB.
 \label{eq:edge-application-linearize}
\end{equation}
\begin{itemize}
 \item \textbf{Proof:} On the edge in
 Equation~\eqref{eq:edge-application-linearize}, $\pi_{{\rm app},2}$ carries
 the application reduction
...
\end{equation}
Equation~\eqref{eq:module-bit-reconstruction} fixes both reconstruction and bit
order.  At this seam a higher-rank target has already been expanded into scalar
...
\begin{align}
 ...
 \label{eq:rel-lmfb}
\end{align}
Equation~\eqref{eq:rel-lmfb} is the exact application-to-core seam relation.
...
\end{equation}
Under Equation~\eqref{eq:edge-bitify}, the witness is retyped as
$\bm b:=\bits(\bm f)$; the same encoded oracle $O$
...
\end{equation}
For the values in Equation~\eqref{eq:paper-column-values}, the verifier
immediately checks the tensor reconstruction
\begin{equation}
...
\end{equation}
Equation~\eqref{eq:paper-grand-product} is the product claim reduced by
$\Pi_1$ below.
...
\end{equation}
\begin{itemize}
 \item \textbf{Proof:} On the edge in Equation~\eqref{eq:edge-pi1}, the paper
 sends $\pi_1=(\mu_c)_c$.  The verifier checks
...
\end{equation}
Equation~\eqref{eq:weighted-coefficient-shape} restricts the public
coefficient; it does not define another relation.
...
\end{equation}
The values in Equation~\eqref{eq:active-chunk-values} determine the proof
fragment
\begin{equation}
 \pi_1^{\rm chunk}
 :=\bigl(\nu^{(\ell)},u^{(\ell)}\bigr)_{
...
\end{equation}
Equation~\eqref{eq:pi1-chunk-proof} lists the transmitted chunk proof fields.
...
\end{equation}
Equation~\eqref{eq:chunk-reconstructs-mu} reconstructs the paper's column
message from the accepted chunk hints.  Inside $\Pi_1$, immediately after
receiving the final active vector, the
verifier checks
\begin{equation}
 \sum_c w'_c\,\pi_q(\mu_c)=y.
 \label{eq:pi1-weighted-reconstruction}
\end{equation}
Equation~\eqref{eq:pi1-weighted-reconstruction} is the tensor-reconstruction
check of
...
\end{equation}
Using Equation~\eqref{eq:chunk-exponent-data}, the single paper target is
realized by the profile family
\begin{equation}
 \bigwedge_{\ell\in\mathcal A}
 \RPESAT(\K,Q_{\Ftwo\!\Z}^{(\ell)},\mathcal C,\Pack).
 \label{eq:chunk-pesat-family}
\end{equation}
Equation~\eqref{eq:chunk-pesat-family} is the profile's chunked realization
of that target.
...
\end{equation}
Using the polynomial in Equation~\eqref{eq:chunk-pesat-polynomial}, the
corresponding named paper relation is the encoded PESAT instance
\begin{align}
 &\RPESAT(\K,Q_{\Ftwo\!\Z}^{(\ell)},\mathcal C,\Pack)
 \notag\\[-2pt]
 &\quad:=
 ...
 \label{eq:rel-gp}
\end{align}
Equation~\eqref{eq:rel-gp} names the resulting encoded PESAT relation.
...
\end{equation}
For the chunked family, the reduction in
Equation~\eqref{eq:edge-gkr-reduction} is applied once per
$\ell\in\mathcal A$.
\begin{itemize}
 ...
 \end{equation}
 Equation~\eqref{eq:gkr-verifier-contract} consumes one canonical GKR
 fragment $\pi_{\rm red}^{(\ell)}$.  Its profile fixes the
 ...
 \end{equation}
 Equation~\eqref{eq:gkr-lin-successor} defines the LIN successor; it is not
 another paper relation.
...
\end{align}
Equation~\eqref{eq:rel-lin-pack} is the linear target consumed by the opening
IOP.

For the profile geometry $n=2^m$, write $\widetilde a$ and
...
MLE-shaped LIN family emitted by $\Pi_{\rm red}$.  A virtual profile first
normalizes a general LIN coefficient with the single LinCheck below.  These
are internal claim transformations, not additional solid edges in the paper's
relation graph.
\begin{enumerate}

\item \textbf{One LinCheck for a general coefficient.}
The direct profile skips this item.  For a general claim
...
\end{equation}
Equation~\eqref{eq:lincheck-round} fixes every round check and challenge
deadline.  Let $r=(r_{\lo},r_{\hi})$, where $|r_{\lo}|=7$.  The first ring-switch
message derives
\begin{equation}
...
\end{equation}
In Equation~\eqref{eq:profile-packed-claim}, the witness is
$P=\Pack(\pi_2(\bm b))$ and $\rho=\Root(O)$ is unchanged.  This step
has error $\varepsilon_{\rm ring}^{(\ell)}$.  For a direct MLE-shaped source,
the verifier checks $z=\mu_\ell$; for the singleton general source normalized
by LinCheck, it checks Equation~\eqref{eq:lincheck-terminal}.  Thus the ring
...
solid edge here: the operations above are the selected profile's
...
\end{align}
Equation~\eqref{eq:rel-lbd-virtual} defines the virtual source relation.
...
\end{align}
Equation~\eqref{eq:rel-virtual-lin} is the partial core's required successor.
...
preserve the direct profile's MLE coefficient shape.  The virtual profile
therefore invokes the one LinCheck in
...
\end{equation}
and prove that evaluator equivalent to the declared full adjoint.  No dense
adjoint vector and no multi-evaluation ring adapter are required.  A profile
without the partial-core predecessor, evaluator, LinCheck ledger, and composed
error rejects; the direct profile continues to reject the virtual source.
...
\end{equation}
Equation~\eqref{eq:composed-soundness} is the profile's composed soundness
bound.  The direct profile sets $\pi_{\rm LC}=()$ and
$\varepsilon_{\rm LC}=0$.
