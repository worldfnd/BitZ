\section{Wire and transcript format v1}
...
The proof system has two physical proof streams, prefix-free typed records, a
...
and the wire version is one.  Changing any normative rule in this section
...
of Equation~\eqref{eq:concrete-domains}.  The public parameters fix the
...
Write $m:=m_f$ and $m_p:=m-7$ for the committed layout.  For the deployed
...
\code{1000--1fff}&---&reserved&These tags \mustnot{} occur in an F2Z proof.\\
\code{2001}&N&chunk $\nu$&$(\ell,\bot,\bot)$; vector of $2^{d_c}$ elements of
$\K$.\\
\code{2081}&H&chunk $\mu$&$(\ell,\bot,\bot)$; vector of $2^{d_c}$ canonical
$u_{128}<2^{127}$.\\
\code{2101}&S&chunk $\zeta$&$(\ell,\bot,i)$ for $0\le i<d_c$; one $\K$
challenge.\\
...
\code{3002--30ff}&GKR&GKR messages&The fixed GKR protocol owns exact
message tags, codecs, and checks in this range.\\
\code{3100--31ff}&GKR&GKR squeezes&The fixed GKR protocol owns exact
squeeze tags and result types in this range.\\
\code{4001}&N&ring $s$ vector&$(\ell,\bot,\bot)$; vector of exactly 128
elements of $\K$.\\
\code{4002}&N&coefficient-LinCheck round&$(\bot,\bot,i)$ for $0\le i<m_f$; exactly the
three coefficients of $g_i(X)=\gamma_{i,0}+\gamma_{i,1}X+\gamma_{i,2}X^2$,
encoded as $\operatorname{Enc}_{\K}(\gamma_{i,0})\parallel
 \operatorname{Enc}_{\K}(\gamma_{i,1})\parallel
 \operatorname{Enc}_{\K}(\gamma_{i,2})$, with no vector prefix.\\
...
\code{4103}&S&coefficient-LinCheck challenge&$(\bot,\bot,i)$ for $0\le i<m_f$; one $\K$
challenge.\\
\code{5001}&D&opening target&$(\bot,\bot,\bot)$;
$\LE{32}{m_f-7}\parallel\operatorname{Enc}_{\K}(\beta)$.  $B$ is recomputed.\\
...
Gap~G1; the rest of that gap remains GKR-protocol work and no protocol
may reassign the tag.  The v1 outer row LinCheck
...
hasher.  It starts empty.  The domain/version frame \code{0001} and statement frame
\code{0002} are absorbed, in that order, before any proof-dependent squeeze.
The application-supplied linear-claim statement digest is
\begin{equation}
...
respectively.  The host application must make the linear-claim statement byte
string in Equation~\eqref{eq:claim-statement-digest} canonical.

The derived core-seam payload is
\begin{equation}
 \mathsf{CoreSeamV1}:=
  \LE{32}{2^{d_r}}\parallel
    \prod_{r=0}^{2^{d_r}-1}\LE{128}{\gamma_r}
  \parallel\LE{32}{2^{d_c}}\parallel
    \prod_{c=0}^{2^{d_c}-1}\LE{128}{\bar a'_c}
  \parallel\LE{128}{\bar\mu},
 \label{eq:core-seam-payload}
\end{equation}
where $\pi_q(\bar a'_c)=a'_c$, $0\le\bar a'_c<q$, and
$\pi_q(\bar\mu)=\mu$, $0\le\bar\mu<q$.  It has exactly
$24+16(2^{d_r}+2^{d_c})$ bytes.  It is absorbed under tag \code{0003} after
the F2Z statement is bound and before the first tag-\code{2001} record.
...
\code{2081} and verifies the exponent bounds and $g^\mu=\nu$ before the first
...
Tags \code{4002} and \code{4103} are enabled only when the pulled-back
coefficient requires the coefficient protocol $\mathsf{LinCheck}_f$.  An
MLE-shaped coefficient requires zero such
events.  Otherwise the verifier alternates, for $i=0,\ldots,m_f-1$, one
tag-\code{4002} read-and-absorb, the check
$g_i(0)+g_i(1)=\sigma_{i-1}$, and one tag-\code{4103} squeeze at the same
scope.  No event may intervene within a round.  It then reads and absorbs the
singleton tag-\code{4001} vector at scope $(0,\bot,\bot)$, derives
$z=\sum_v\eq(v,r_{\lo})s_v$, and checks
$\sigma_{m_f-1}=\widetilde a_f(r)z$ before tag \code{4101}.  The coefficient
evaluator is verifier-derived and contributes no payload.  This is the only
coefficient LinCheck in the opening path; the same tag-\code{4001} vector continues into
the inherited ring-switch computation.
...
\end{equation}
Equation~\eqref{eq:sumcheck-message-kinds} is the complete tag-
\code{5004} kind registry.  Each payload is the one-byte kind followed by the
two canonical $\K$
elements in the registry.  Any other kind/scope pair rejects.

The outer $\mathsf{VerifyF2Z}$ caller first requires that $B$ is the
deterministic coefficient function derived in P9/V9, with domain size
...
unknown version or tag, a reserved bit, wrong endian or channel, record
...
or extra recursive fields.  A coefficient-LinCheck-enabled fixture must additionally
cover a wrong round polynomial, reordered \code{4002/4103} pair, altered
terminal coefficient, and failed terminal gate.  Native, recursive, and independent reference
...
known-answer set for the primitive container/frame codec,
