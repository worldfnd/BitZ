...
The domain registry is below.  Domain codes and scopes are verifier-derived
transcript inputs, not record headers.  \textsf{D} events are public or
derived, have no proof record, and are absorbed at their ledger position;
\textsf{N} payloads occur only in $N$ and are absorbed;
\textsf{H} payloads occur only in $H$ and are checked but never absorbed;
...
\code{3002--30ff}&Sub.&subprotocol messages&The fixed subordinate ledger
owns exact message domain codes, codecs, and checks for $\pi_{\rm GKR}$
followed by $\pi_{\rm MLE}$.\\
...
may reassign the domain code.  The v1 outer row LinCheck
omits the redundant \code{presum.claimed\_sums} field and derives
$[e_d+1]$, so domain code \code{3081} is reserved and \mustnot{} occur.
