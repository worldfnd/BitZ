...
ZeroCheck protocol, GKR protocol, constrained-code construction, transcript,
and opening protocol.  None is selected or negotiated by proof bytes.  A proof system
without byte-exact ZeroCheck and GKR ledgers or without a canonical codeword
construction and root vectors is incomplete and \mustnot{} accept proofs.
...
\code{1000--1fff}&ZC&ZeroCheck&The fixed ZeroCheck protocol owns a
disjoint, byte-exact ledger within this range.\\
...
elements of $\K$.\\
\code{4002}&N&LinCheck round&$(\bot,\bot,i)$ for $0\le i<m$; exactly the
...
\code{4103}&S&LinCheck challenge&$(\bot,\bot,i)$ for $0\le i<m$; one $\K$
challenge.\\
...
The ZeroCheck and GKR ranges are black-box namespaces, not permission to use
implementation-defined bytes.  The fixed protocols must publish ledgers with
the same columns as this table.  The fixed
...
The source-statement digest is
\begin{equation}
 d_{\rm src}:=\operatorname{BLAKE3}_{256}
 \bigl(\operatorname{hex}^{-1}
   (\code{46325a2d5043532f6170702d73746174656d656e742f763100})
 \parallel\mathsf{source\_statement\_bytes}\bigr).
 \label{eq:source-statement-digest}
\end{equation}
For v1 byte compatibility, Equation~\eqref{eq:source-statement-digest} retains
...
respectively.  ZeroCheck must make the source-statement byte string in
Equation~\eqref{eq:source-statement-digest} canonical.
...
Tags \code{4002} and \code{4103} are enabled only when the fixed core mode
declares a general-coefficient LinCheck; the direct mode requires zero such
events.  The virtual mode alternates, for $i=0,\ldots,m-1$, one
...
$\sigma_{m-1}=\widetilde a(r)z$ before tag \code{4101}.  The coefficient
evaluator is verifier-derived and contributes no payload.  This is the only
LinCheck in the virtual path; the same tag-\code{4001} vector continues into
...
deterministic coefficient function derived in P10/V10, with domain size
...
or extra recursive fields.  A LinCheck-enabled fixture must additionally
