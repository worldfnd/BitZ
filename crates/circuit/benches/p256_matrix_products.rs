//! Witgen, matrix projections, and Wengert `r(A+xB+x²C)` for P-256 ECDSA.
//!
...
fn report_m_transpose(transpose: &MaterializedMTranspose) {
    ...
    REPORT_M_TRANSPOSE.call_once(|| {
        eprintln!(
            "P-256 ECDSA M (stored as M^T): {bytes} bytes ({:.2} MiB), {} nonzeros, \
             {} M rows x {} M columns",
...
fn build_wengert_tape() -> WengertTape {
    let mut generator = WengertGenerator::new(VERIFY_DIGEST_INPUT_BITS);
    let inputs = generator.take_boxed_inputs();
    verify_digest_circuit(&mut generator, &inputs);
    generator.finish()
}
...
fn report_wengert_tape(tape: &WengertTape) {
    ...
    REPORT_WENGERT_TAPE.call_once(|| {
        eprintln!(
            "P-256 ECDSA Wengert tape: {bytes} bytes ({:.2} MiB), {} nodes, {} edges, \
...
#[divan::bench]
fn p256_abc_wengert_tape(bencher: Bencher) {
    prepare();
