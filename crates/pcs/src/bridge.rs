//! Zero-copy conversion between the local field and the private backend field.
...
const _: () = {
    ...
    let _ = BackendF128 { lo: 0u64, hi: 0u64 };
    assert!(size_of::<LocalF128>() == 16);
    assert!(size_of::<BackendF128>() == 16);
    assert!(align_of::<LocalF128>() == 16);
    assert!(align_of::<BackendF128>() == 16);
    assert!(offset_of!(LocalF128, lo) == 0);
    assert!(offset_of!(BackendF128, lo) == 0);
    assert!(offset_of!(LocalF128, hi) == 8);
    assert!(offset_of!(BackendF128, hi) == 8);
};

/// Views local field elements as backend field elements without copying.
///
...
pub(crate) fn as_backend_f128s(values: &[LocalF128]) -> &[BackendF128] {
...
mod tests {
    ...
    fn cast_preserves_pointer_length_and_words() {
        ...
        let cast = as_backend_f128s(&values);

        assert_eq!(cast.as_ptr().cast::<LocalF128>(), values.as_ptr());
        assert_eq!(cast.len(), values.len());
        for (local, backend) in values.iter().zip(cast) {
            assert_eq!((backend.lo, backend.hi), (local.lo, local.hi));
        }
    }

    #[test]
    fn cast_accepts_an_empty_slice() {
        assert!(as_backend_f128s(&[]).is_empty());
    }
}
